...
const FundraisingList = ({route,navigation}) => {

    const myEmail=route.params.myEmail; 
    const [productdata,setproductdata] = useState("");
   
    var fetchInsertApi2="https://arzachel.org/business_grow/investor/getallfundlist.php";
        var headers={
         ...
         renderItem={(element)=>{
             return (
              ...
              <View style={styles.container}> 
               ...
               <Title style={{color:'black',paddingTop:5,fontSize:15,fontWeight:'bold'}}> &nbsp;&nbsp;Business Name : {element.item.business_name}</Title>
             
               <Paragraph  style={{color:'black',marginLeft:9}}> Category : {element.item.b_category}{'\n'}
               Email Address : {element.item.user_email}{'\n'}
                Detail : {element.item.business_detail}{'\n'}
                Amount Required : Rs {element.item.amount_required}{'\n'}
                Share Equity : {element.item.share_equity} %</Paragraph>
               </View>
               </View>
